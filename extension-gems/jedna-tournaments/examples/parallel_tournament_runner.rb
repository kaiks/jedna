...
class ParallelTournamentRunner
  ...
  def merge_all_results
    ...
    @processes.times do |i|
      ...
      parse_results_file(results_file, total_wins) if File.exist?(results_file)
      ...
      File.readlines(game_log_file).each do |line|
        ...
        # Only count actual game results, not individual plays
        total_games += 1 if line.start_with?('Game ')
      end
    end
  ...
  def parse_results_file(file, total_wins)
    ...
    # Parse match results
    return unless content =~ /Match: (\w+) vs (\w+)/

    agent1 = ::Regexp.last_match(1)
    agent2 = ::Regexp.last_match(2)

    # Extract wins
    total_wins[agent1] += ::Regexp.last_match(1).to_i if content =~ /#{agent1}: (\d+) wins/
    return unless content =~ /#{agent2}: (\d+) wins/

    total_wins[agent2] += ::Regexp.last_match(1).to_i
  end
