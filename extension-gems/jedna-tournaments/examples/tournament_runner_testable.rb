...
# Testable Tournament Runner - Refactored for testability
#
# This version separates concerns for better testing:
# - TournamentConfig: Configuration management
# - GameEngine: Single game execution with Jedna integration
# - TournamentScheduler: Tournament format logic
# - ResultsTracker: Statistics and result tracking
# - TournamentOrchestrator: Main coordination
#
# Usage: ruby tournament_runner_testable.rb <config.yaml>
...
class GameEngine
  ...
  def setup_game(player1_name, player2_name, agent_map)
  ...
  def handle_turn(game, player, agent)
    return unless agent
  ...
  def handle_game_end(game, agent_map)
    winner = game.winner
...
class TournamentOrchestrator
  ...
  def run_elimination_bracket
    ...
    brackets.each do |bracket|
      ...
      bracket[:matches].each do |match|
        if match[:type] == :bye
          log_output "#{match[:player1]} gets a bye"
          next_round << match[:player1]
          next
        end

        # Run actual match
        actual_match = remaining_agents.each_slice(2).find do |p1, p2|
          [p1, p2].sort == [match[:player1], match[:player2]].sort
        end

        next unless actual_match

        winner = run_elimination_match(actual_match[0], actual_match[1], bracket[:round])
        next_round << winner
      end
  ...
  def run_elimination_match(player1, player2, round_num)
    ...
    match_winner = wins.max_by { |_, v| v }[0]
    loser = [player1, player2].find { |p| p != match_winner }

    @results_tracker.record_match(player1, player2, wins, match_winner)
    @results_tracker.mark_elimination(loser, round_num)

    log_output "  Winner: #{match_winner} (#{wins[match_winner]}-#{wins[loser]})"
