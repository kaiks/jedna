...
class SmarterAgent
  ...
  def decide_action(state)
    ...
    if playable_cards.any?
      ...
      if min_opponent_cards == 1
        ...
        if !disruptive && playable_cards.include?('wd4')
          return play_card('wd4', hand)
        end
      ...
      non_wild_cards = playable_cards.reject { |c| c == 'w' || c == 'wd4' }
      ...
      if non_wild_cards.any?
        ...
        if min_opponent_cards == 1 && action_cards.any?
        ...
        if min_opponent_cards >= 4
          ...
          if number_cards.any?
            isolated_numbers = number_cards.select do |card|
              ...
              is_isolated = !hand.any? { |h| h != card && h.end_with?(figure) }
              is_isolated
            end
            
            # Play isolated number cards first when opponent has many cards
            if isolated_numbers.any?
              return play_card(isolated_numbers.first, hand)
            end
          end
          
          # If no isolated numbers but we have any numbers, prefer them over action cards
          if number_cards.any? && action_cards.any?
            return play_card(number_cards.first, hand)
          end
        end
  ...
  def find_best_chain_starter(playable_cards, hand)
    ...
    playable_cards.each do |card|
      ...
      if score > best_score
        ...
        if card.match?(/^[rbgy][0-9]$/) && !best_card.match?(/^[rbgy][0-9]$/)
          best_card = card
        end
      end
    end
  ...
  def calculate_simple_chain_score(card, hand)
    ...
    remaining_hand.each do |h|
      ...
      if h_color == color || h_figure == figure
        direct_follows += 1
        
        # For number cards matching by figure, check if they lead to more plays
        if h_figure == figure && h_color != color && card.match?(/^[rbgy][0-9]$/)
...
SmarterAgent.new.run if __FILE__ == $0