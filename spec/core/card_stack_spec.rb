...
RSpec.describe Jedna::CardStack do
  let(:stack) { Jedna::CardStack.new }
  ...
  describe '#fill' do
    ...
    it 'creates one zero of each color' do
      ...
      expect(zeros.map(&:color).sort).to eq(%i[blue green red yellow])
    end
    ...
    it 'creates wild cards with :wild color' do
      wilds = stack.select(&:special_card?)
      expect(wilds).to all(have_attributes(color: :wild))
    end

    it 'returns self for chaining' do
      new_stack = Jedna::CardStack.new
  ...
  describe '#pick' do
    before do
      ...
      @red5 = Jedna::Card.new(:red, 5)
      @blue3 = Jedna::Card.new(:blue, 3)
      @green_skip = Jedna::Card.new(:green, 'skip')
    ...
    it 'returns CardStack instance' do
      picked = stack.pick(1)
      expect(picked).to be_a(Jedna::CardStack)
    end
    ...
    it 'returns empty CardStack when picking from empty stack' do
      empty_stack = Jedna::CardStack.new
  ...
  describe '#create_discard_pile' do
    it 'creates a Hand instance for discard pile' do
      ...
      expect(discard).to be_a(Jedna::Hand)
      expect(discard).to be_empty
    end
  end

  describe 'inheritance from Hand' do
    it 'supports all Jedna::Hand methods' do
      stack << Jedna::Card.new(:red, 5)
  ...
  describe 'integration with UNO deck' do
    ...
    it 'allows dealing initial hands' do
      ...
      hands.each do |hand|
        ...
        expect(hand).to be_a(Jedna::CardStack)
      end
    end
  end
end
