...
RSpec.describe "Jedna::Renderer" do
  let(:red_5) { Jedna::Card.new(:red, 5) }
  let(:blue_skip) { Jedna::Card.new(:blue, 'skip') }
  let(:wild_card) { Jedna::Card.new(:wild, 'wild') }
  let(:wild_draw_4) { Jedna::Card.new(:wild, 'wild+4') }
  let(:hand) { Jedna::Hand.new([red_5, blue_skip, wild_card]) }
  
  describe Jedna::TextRenderer do
    let(:renderer) { Jedna::TextRenderer.new }
    ...
    describe '#render_hand' do
      ...
      it 'handles empty hands' do
        expect(renderer.render_hand(Jedna::Hand.new)).to eq('')
      end
    end
  ...
  describe Jedna::IrcRenderer do
    let(:renderer) { Jedna::IrcRenderer.new }
    ...
    describe '#render_card' do
      ...
      it 'uses correct color codes' do
        ...
        expect(renderer.render_card(Jedna::Card.new(:green, 7))).to include("\x033")    # green
        expect(renderer.render_card(Jedna::Card.new(:yellow, 9))).to include("\x037")   # yellow
  ...
  describe Jedna::HtmlRenderer do
    let(:renderer) { Jedna::HtmlRenderer.new }
