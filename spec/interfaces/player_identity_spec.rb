...
RSpec.describe "Jedna::PlayerIdentity" do
  describe Jedna::IrcIdentity do
  ...
  describe Jedna::SimpleIdentity do
    let(:identity) { Jedna::SimpleIdentity.new('Player1') }
    ...
    describe '#matches?' do
      it 'matches another SimpleIdentity with same name' do
        other = Jedna::SimpleIdentity.new('Player1')
        expect(identity.matches?(other)).to be true
      end
  ...
  describe Jedna::UuidIdentity do
    ...
    describe '#display_name' do
      ...
      it 'generates a name if none provided' do
        identity_no_name = Jedna::UuidIdentity.new(uuid)
    ...
    describe '#matches?' do
      it 'matches another UuidIdentity with same UUID' do
        other = Jedna::UuidIdentity.new(uuid, 'Bob')
        expect(identity.matches?(other)).to be true
      end
      
      it 'does not match UuidIdentity with different UUID' do
        other = Jedna::UuidIdentity.new('different-uuid')
        expect(identity.matches?(other)).to be false
      end
